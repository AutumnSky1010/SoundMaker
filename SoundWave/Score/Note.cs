...
public class Note : SoundComponentBase
{

	public Note(Scale scale, uint scaleNumber, int length, bool isDotted = false)
    ...
    public override ushort[] GetSquareWave(FormatChunk format, SquareWaveRatio squareWaveRatio, int tempo, int length)
    {
        ...
        while (count <= length)
        {
            int allRepeatTimes = (int)(format.SamplingFrequency / this.Hertz);
    ...
    public override ushort[] GetTriangleWave(FormatChunk format, int tempo)
    {
        ...
        return GetTriangleWave(format, tempo, listLength);
    }
    ...
    public override ushort[] GetTriangleWave(FormatChunk format, int tempo, int length)
	{
        ...
        {
            // △の波形の波形を作るための繰り返し回数
            double repeatNumber = format.SamplingFrequency / this.Hertz;
            // 直線の方程式の傾きを求める。
